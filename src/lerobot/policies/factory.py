...
def add_delta_state_processor_if_needed(processor_pipeline: PolicyProcessorPipeline[Any, Any], policy_cfg, is_preprocess: bool) -> PolicyProcessorPipeline[Any, Any]:
    ...
    if isinstance(policy_cfg, SmolVLAConfig):
        if is_preprocess:
            index = len(new_steps) - 1  # Before the last normalization step
        else:
            index = 1  # After the first unnormalization step
    ...
    **kwargs: Unpack[ProcessorConfigKwargs],
    ...
    if pretrained_path:
        ...
        return (
            ...
            ), policy_cfg=policy_cfg, is_preprocess=True),
            add_delta_state_processor_if_needed(PolicyProcessorPipeline.from_pretrained(
                pretrained_model_name_or_path=pretrained_path,
                config_filename=kwargs.get(
            ...
            ), policy_cfg=policy_cfg, is_preprocess=False),
        )
